...
mo_kline(struct Client *client_p, struct Client *source_p, int parc, char **parv)
{
	...
	if(already_placed_kline(source_p, user, host, tkline_time, 0))
		return;
...
ms_kline(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
	...
	if(find_cluster(source_p->user->server, CLUSTER_KLINE))
	{
		...
		if(already_placed_kline(source_p, kuser, khost, tkline_time, 1))
			return;
			    ...
			    source_p->username, source_p->host, OPER_K))
	{
		...
		if(already_placed_kline(source_p, kuser, khost, tkline_time, 0))
			return;
 ...
 */
static int
valid_user_host(char *luser, char *lhost)
{
