	...
	DLINK_FOREACH(ptr, serv_list.head)
	{
		...
		if(IsCapable(target_p, CAP_ZIP))
		{
			...
			if(!target_p->localClient->slinkq)
			{
				...
				comm_setselect(target_p->localClient->ctrlfd, FDLIST_SERVER, COMM_SELECT_WRITE, send_queued_slink_write, target_p, 0);
			}
		}
	}
}
...
start_io(struct Client *server)
{
	...
	/* schedule a write */
	comm_setselect(server->localClient->ctrlfd, FDLIST_SERVER, COMM_SELECT_WRITE, send_queued_slink_write, server, 0);
}
