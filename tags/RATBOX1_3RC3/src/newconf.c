...
conf_set_oper_global_kill(void *data)
{
	...
	if(yesno)
		yy_achead->port |= (OPER_GLOBAL_KILL|OPER_LOCAL_KILL);
	else
		yy_achead->port &= ~(OPER_GLOBAL_KILL|OPER_LOCAL_KILL);
}
...
conf_set_oper_local_kill(void *data)
{
	...
	if(yesno)
		yy_achead->port |= OPER_LOCAL_KILL;
	else
		yy_achead->port &= ~ OPER_LOCAL_KILL;
}
...
newconf_init()
{
	...
	add_conf_item("general", "gline_min_cidr", CF_YESNO, conf_set_general_gline_min_cidr);
	add_conf_item("general", "gline_min_cidr6", CF_YESNO, conf_set_general_gline_min_cidr6);
