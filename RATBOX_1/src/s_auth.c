...
#ifdef IPV6   
        if(ConfigFileEntry.fallback_to_ip6_int && auth->client->localClient->aftype == AF_INET6
                && auth->ip6_int == 0)
        {
                MyFree(reply);
		reply = NULL;
                if(adns_getaddr(&auth->client->localClient->ip,
                                auth->client->localClient->aftype, 
                                &auth->dns_query, 0)) 
                {
...
#ifdef IPV6
                if(ConfigFileEntry.fallback_to_ip6_int && auth->client->localClient->aftype == AF_INET6 &&
                  !adns_getaddr(&auth->client->localClient->ip, client->localClient->aftype,
                                &auth->dns_query, 0))
                {
                        SetDNSPending(auth);
                } else            
