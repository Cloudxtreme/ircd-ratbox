		...
		const char *gecos, int opered)
{
	...
	fake_p = BlockHeapAlloc(client_heap);
	memset(fake_p, 0, sizeof(struct Client));

	make_user(fake_p);

	fake_p->localClient = (struct LocalUser *) BlockHeapAlloc(lclient_heap);
	memset(fake_p->localClient, 0, sizeof(struct LocalUser));

	fake_p->from = fake_p;

	strlcpy(fake_p->name, name, sizeof(fake_p->name));
	strlcpy(fake_p->username, username, sizeof(fake_p->username));
	strlcpy(fake_p->host, host, sizeof(fake_p->host));
	strlcpy(fake_p->info, gecos, sizeof(fake_p->info));

	fake_p->hopcount = 0;
	fake_p->tsinfo = 1;
	fake_p->flags = FLAGS_FAKE;
	fake_p->localClient->fd = -1;
	...
	add_to_client_hash(fake_p->name, fake_p);

	SetClient(fake_p);
	SetDead(fake_p);

	dlinkAddTail(fake_p, &fake_p->node, &global_client_list);

	introduce_client(NULL, fake_p, fake_p->user, fake_p->name);
...
destroy_fake_client(struct Client *fake_p)
{
	...
	del_from_client_hash(fake_p->name, fake_p);

	dlinkDelete(&fake_p->node, &global_client_list);
	free_user(fake_p->user, fake_p);
	BlockHeapFree(lclient_heap, fake_p->localClient);
	BlockHeapFree(client_heap, fake_p);
}
...
create_fake_server(const char *name, const char *gecos, int persist)
{
	...
	fake_p->localClient = BlockHeapAlloc(lclient_heap);
	memset(fake_p->localClient, 0, sizeof(struct LocalUser));
	...
	dlinkAddTail(fake_p, &fake_p->node, &global_client_list);
	dlinkAddTailAlloc(fake_p, &global_serv_list);
...
destroy_fake_server(struct Client *fake_p, int send_squit)
{
	...
	dlinkDelete(&fake_p->node, &global_client_list);
	dlinkFindDestroy(&global_serv_list, fake_p);
