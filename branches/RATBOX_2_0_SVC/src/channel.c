...
add_user_to_channel(struct Channel *chptr, struct Client *client_p, int flags)
{
    ...
    if(msptr->flags & CHFL_CHANOP)
		dlinkAdd(msptr, &msptr->channode, &chptr->members[1]);
	else
		dlinkAdd(msptr, &msptr->channode, &chptr->members[0]);
...
remove_user_from_channel(struct membership *msptr)
{
	...
	if(msptr->flags & CHFL_CHANOP)
		dlinkDelete(&msptr->channode, &chptr->members[1]);
	else
		dlinkDelete(&msptr->channode, &chptr->members[0]);
	...
	if(dlink_list_length(&chptr->members[0]) + dlink_list_length(&chptr->members[1]) <= 0)
...
channel_member_names(struct Channel *chptr, struct Client *client_p, int show_eon)
{
	...
	if(ShowChannel(client_p, chptr))
	{
		...
		for (i = 0; i < 2; i++)
		{
		DLINK_FOREACH(ptr, chptr->members[i].head)
		{
			msptr = ptr->data;
...
can_join(struct Client *source_p, struct Channel *chptr, char *key)
{
	...
	if(chptr->mode.limit && 
	   dlink_list_length(&chptr->members[0]) + dlink_list_length(&chptr->members[1]) >= chptr->mode.limit)
