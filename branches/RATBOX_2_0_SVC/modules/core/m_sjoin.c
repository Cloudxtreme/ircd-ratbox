...
static void remove_our_invites(struct Channel *chptr);
...
remove_our_modes(struct Channel *chptr, struct Client *source_p)
{
	...
	*mbuf++ = '-';
	...
	for(j = 0; j < 2; j++)
	{
	DLINK_FOREACH_SAFE(ptr, next_ptr, chptr->members[j].head)
	{
		msptr = ptr->data;
		...
		if(is_chanop(msptr))
		{
			...
			dlinkMoveNode(&msptr->channode, &chptr->members[1], &chptr->members[0]);
...
static void
remove_our_invites(struct Channel *chptr)
{
	struct Client *target_p;
	dlink_node *ptr, *next_ptr;
	...
	DLINK_FOREACH_SAFE(ptr, next_ptr, chptr->invites.head)
	{
		...
		dlinkFindDestroy(&target_p->user->invited, chptr);
		dlinkDestroy(ptr, &chptr->invites);
	}
}
