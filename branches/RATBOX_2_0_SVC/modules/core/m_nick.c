...
change_local_nick(struct Client *client_p, struct Client *source_p, char *nick)
{
	...
	/* dont reset TS if theyre changing between case of the same nick */
		...
		sendto_realops_flags(UMODE_ALL, L_ALL,
			...
			if(IsFake(target_p))
			{
				kill_client_serv_butone(NULL, target_p, 
							"%s (Nickname Collision)", me.name);
				introduce_client(NULL, target_p, target_p->user, target_p->name);
				hook_call_event(h_fake_collision_id, target_p);
				return 0;
			}
#endif
