...
/* we store ipv6 ips for remote clients, needs to be v6 always */
...
#define FLAGS_FAKE         0x400000
#define FLAGS_FAKEPERSIST  0x800000
#endif
...
#ifdef ENABLE_FAKE_CLIENTS
extern struct Client *create_fake_client(const char *name, const char *username,
					const char *host, const char *gecos, int);
extern void destroy_fake_client(struct Client *);

extern struct Client *create_fake_server(const char *name, const char *gecos, int persist);
extern void destroy_fake_server(struct Client *, int send_squit);
#endif
