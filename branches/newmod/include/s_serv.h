...
typedef void SlinkRplHnd(unsigned int replyid, unsigned int datalen,
			 unsigned char *data, struct rpi_client *client_p);
struct SlinkRplDef
{
...
extern int hunt_server(struct rpi_client *client_pt,
		       struct rpi_client *source_pt,
		       const char *command, int server, int parc, const char **parv);
extern void send_capabilities(struct rpi_client *, int);
extern const char *show_capabilities(struct rpi_client *client);
extern void try_connections(void *unused);
extern void start_collect_zipstats(void);
extern void collect_zipstats(void *unused);

extern int check_server(const char *name, struct rpi_client *server);
extern int server_estab(struct rpi_client *client_p);

extern int serv_connect(struct server_conf *, struct rpi_client *);
