...
static void client_dopacket(struct rpi_client *client_p, char *buffer, size_t length);
 ...
 */
static void
parse_client_queued(struct rpi_client *client_p)
{
	int dolen = 0;
	int checkflood = 1;
 ...
 */
void
flood_endgrace(struct rpi_client *client_p)
{
...
flood_recalc(int fd, void *data)
{
	...
	struct rpi_localclient *lclient_p = client_p->localClient;
...
{
	struct rpi_client *server = data;
	struct rpi_localclient *lserver = server->localClient;
 ...
 */
void
client_dopacket(struct rpi_client *client_p, char *buffer, size_t length)
{
