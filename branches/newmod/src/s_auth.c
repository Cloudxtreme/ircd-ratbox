...
init_auth(void)
{
	/* This hook takes a struct rpi_client for its argument */
...
make_auth_request(struct rpi_client *client)
{
...
{
	struct rpi_client *client = auth->client;
...
start_auth_query(struct AuthRequest *auth)
{
	...
	if((fd = comm_open(family, SOCK_STREAM, 0, "ident")) == -1)
	{
		report_error("creating auth stream socket %s:%s",
			     auth->client->v->get_name(auth->client, SHOW_IP),
			     auth->client->v->log_name(auth->client, SHOW_IP), errno);
		++ServerStats->is_abad;
		return 0;
	}
	...
	if(!comm_set_nb(fd))
	{
		report_error(NONB_ERROR_MSG, auth->client->v->get_name(auth->client, SHOW_IP),
				auth->client->v->log_name(auth->client, SHOW_IP), errno);
		fd_close(fd);
		return 0;
	}
 ...
 */
void
start_auth(struct rpi_client *client)
{
...
auth_connect_callback(int fd, int error, void *data)
{
			  ...
			  (struct sockaddr *) &them, (socklen_t *) & tlen))
	{
		ilog(L_IOERROR, "auth get{sock,peer}name error for %s:%m",
		     auth->client->v->log_name(auth->client, SHOW_IP));
		auth_error(auth);
		return;
	}
 ...
 */
void
delete_auth_queries(struct rpi_client *target_p)
{
	struct AuthRequest *auth;
