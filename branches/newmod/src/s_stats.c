 ...
 */
void
tstats(struct rpi_client *source_p)
{
...
void
count_memory(struct rpi_client *source_p)
{
	struct rpi_client *target_p;
	...
	sendto_one_numeric(source_p, RPL_STATSDEBUG,
			   ...
			   users_counted,
			   (unsigned long) users_counted * sizeof(struct rpi_user),
			   users_invited_count,
	...
	sendto_one_numeric(source_p, RPL_STATSDEBUG,
			   ...
			   (long)wwu, (long) (wwu * sizeof(struct rpi_user)));

	sendto_one_numeric(source_p, RPL_STATSDEBUG,
			   "z :Whowas array %u(%d)",
			   NICKNAMEHISTORYLENGTH, (int) wwm);

	totww = wwu * sizeof(struct rpi_user) + wwm;
