...
static void start_io(struct rpi_client *server);
...
#ifndef __VMS
static int fork_server(struct rpi_client *client_p);
#endif

static CNCB serv_connect_callback;

void
slink_error(unsigned int rpl, unsigned int len, unsigned char *data, struct rpi_client *server_p)
{
...
void
slink_zipstats(unsigned int rpl, unsigned int len, unsigned char *data, struct rpi_client *server_p)
{
 ...
 */
int
hunt_server(struct rpi_client *client_p, struct rpi_client *source_p,
	    const char *command, int server, int parc, const char *parv[])
{
	...
	if(!target_p)
	{
		...
		else
		{
			...
			{
				if(match(new, ((struct rpi_client *) (ptr->data))->name))
				{
					target_p = ptr->data;
					break;
				}
...
int
check_server(const char *name, struct rpi_client *client_p)
{
	struct server_conf *server_p = NULL;
	struct server_conf *tmp_p;
	dlink_node *ptr;
 ...
 */
void
send_capabilities(struct rpi_client *client_p, int cap_can_send)
{
 ...
 */
static void
burst_modes_TS5(struct rpi_client *client_p, char *chname, dlink_list *list, char flag)
{
	dlink_node *ptr;
	struct Ban *banptr;
 ...
 */
static void
burst_modes_TS6(struct rpi_client *client_p, struct Channel *chptr,
 ...
 */
static void
burst_TS5(struct rpi_client *client_p)
{
 ...
 */
static void
burst_TS6(struct rpi_client *client_p)
{
	static char modebuf[MODEBUFLEN];
	static char parabuf[MODEBUFLEN];
	static char ubuf[12];
 ...
 *
 * inputs       - pointer to an struct rpi_client
 * output       - pointer to static string
 * side effects - build up string representing capabilities of server listed
 */
const char *
show_capabilities(struct rpi_client *target_p)
{
 ...
 * server_estab
 *
 * inputs       - pointer to a struct rpi_client
 * output       -
 * side effects -
 */
int
server_estab(struct rpi_client *client_p)
{
	struct rpi_client *target_p;
	...
	if(!comm_set_buffers(client_p->localClient->fd, READBUF_SIZE))
		report_error(SETBUF_ERROR_MSG,
			     ...
			     client_p->v->log_name(client_p, SHOW_IP), errno);
	...
	ilog(L_SERVER, "Link with %s established: (%s) link",
	     client_p->v->log_name(client_p, SHOW_IP), show_capabilities(client_p));
...
static void
start_io(struct rpi_client *server)
{
	unsigned char *iobuf;
	int c = 0;
	int linecount = 0;
	...
	else
	{
		...
		if(!comm_set_nb(server->localClient->fd))
		{
			report_error(NONB_ERROR_MSG,
					server->v->server_name(server, SHOW_IP),
					server->v->log_name(server, SHOW_IP),
					errno);
		}
 ...
 */
int
serv_connect(struct server_conf *server_p, struct rpi_client *by)
{
	struct rpi_client *client_p;
	...
	if((client_p = find_server(NULL, server_p->name)))
	{
		sendto_realops_flags(UMODE_ALL, L_ALL,
				     ...
				     server_p->name, client_p->v->server_name(client_p, SHOW_IP));
...
{
	struct rpi_client *client_p = data;
	struct server_conf *server_p;
