...
dlink_list classes; // list<class_holder>
dlink_list vtables; // list<vtable_holder>

void inc_ref (void *ctx)
{
        _rpi_base *obj = (_rpi_base *) ctx;
        ++obj->_rpi_ref_count;
}

void dec_ref (void *ctx)
{
        _rpi_base *obj = (_rpi_base *) ctx;
        if (--obj->_rpi_ref_count <= 0) {
                obj->v->_rpi_destroy(ctx);
        }
}

struct _rpi_base_vtable vtable = {
	0,
	inc_ref,
	dec_ref,
	NULL
...
void *new (char const *name, int flags, ...)
{
	va_list ap;
	dlink_node *ptr;
	class_holder *c = NULL;
	vtable_holder *v = NULL;
	_rpi_base *bobj;

	DLINK_FOREACH(ptr, classes.head) {
       	if (!strcmp(((class_holder*)ptr->data)->name, name)) {
			c = ptr->data;
			break;
		}
	}
	if (!c)
		return NULL;
	va_start(ap, flags);
	bobj = (_rpi_base *) c->ctor(flags, ap);
	...
	bobj->_rpi_ref_count = 0;

	DLINK_FOREACH(ptr, vtables.head) {
		if (!strcmp(((vtable_holder*)ptr->data)->name, name)) {
			v = ptr->data;
			break;
		}
	}
...
{
	_rpi_base *ctx = (_rpi_base *) obj;
	ctx->v->_rpi_dec_ref(ctx);
}

rpi_bool register_class (char const *name, rpi_ctor ctor, rpi_dtor dtor)
{
	class_holder *c = malloc(sizeof(class_holder));
	vtable_holder *v = malloc(sizeof(vtable_holder));

	_rpi_base_vtable *vt = malloc(sizeof(_rpi_base_vtable));

	c->ctor = ctor;
	c->name = name;

	memcpy(vt, &vtable, sizeof vtable);
	vt->_rpi_destroy = dtor;
	vt->_rpi_rtti_id = 0;
	v->vtable = vt;
	v->name = name;

	dlinkAddAlloc(v, &vtables);
	dlinkAddAlloc(c, &classes);
	return rpi_true;
}
...
static int mo_modload(struct rpi_client *, struct rpi_client *, int, const char **);
static int mo_modlist(struct rpi_client *, struct rpi_client *, int, const char **);
static int mo_modreload(struct rpi_client *, struct rpi_client *, int, const char **);
static int mo_modunload(struct rpi_client *, struct rpi_client *, int, const char **);
static int mo_modrestart(struct rpi_client *, struct rpi_client *, int, const char **);
...
/* load a module .. */
static int
mo_modload(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char **parv)
{
	char *m_bn;
...
/* unload a module .. */
static int
mo_modunload(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char **parv)
{
	char *m_bn;
	int modindex;
...
static int
mo_modreload(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char **parv)
{
	char *m_bn;
	int modindex;
	int check_core;
...
/* list modules .. */
static int
mo_modlist(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char **parv)
{
	int i;
...
static int
mo_modrestart(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char **parv)
{
	int modnum;
