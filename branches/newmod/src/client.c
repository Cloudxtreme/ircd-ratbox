...
static int exit_remote_client(struct rpi_client *, struct rpi_client *, struct rpi_client *,const char *);
static int exit_remote_server(struct rpi_client *, struct rpi_client *, struct rpi_client *,const char *);
static int exit_local_client(struct rpi_client *, struct rpi_client *, struct rpi_client *,const char *);
static int exit_unknown_client(struct rpi_client *, struct rpi_client *, struct rpi_client *,const char *);
static int exit_local_server(struct rpi_client *, struct rpi_client *, struct rpi_client *,const char *);
static int qs_server(struct rpi_client *, struct rpi_client *, struct rpi_client *, const char *comment);

/* RPI functions */
void 			rpi_client_close_connection		(struct rpi_client *client_p);
const char *	rpi_client_get_name				(struct rpi_client *client, int show_ip);
int 			rpi_client_accept_message		(struct rpi_client *, struct rpi_client *);
void 			rpi_client_del_from_accept		(struct rpi_client *, struct rpi_client *);
void 			rpi_client_del_all_accepts		(struct rpi_client *);
const char *	rpi_client_server_name			(struct rpi_client *, int);
const char *	rpi_client_log_name				(struct rpi_client *, int);
void			rpi_client_dead_link			(struct rpi_client *);
int				rpi_client_show_ip				(struct rpi_client *, struct rpi_client *);
...
init_client(void)
{
	 ...
	 */
	client_heap = BlockHeapCreate(sizeof(struct rpi_client), CLIENT_HEAP_SIZE);
	lclient_heap = BlockHeapCreate(sizeof(struct rpi_localclient), LCLIENT_HEAP_SIZE);
	...
	register_class("client", client_ctor, client_dtor);
}
...
static struct _rpi_client_vtable client_vtable = {
	rpi_client_close_connection,
	rpi_client_get_name,
	rpi_client_accept_message,
	rpi_client_del_from_accept,
	rpi_client_del_all_accepts,
	rpi_client_server_name,
	rpi_client_log_name,
	rpi_client_dead_link,
	rpi_client_show_ip,
};

void *
client_ctor(int flags, va_list ap)
{
	rpi_client *from = va_arg(ap, rpi_client *);
	rpi_client *client_p = NULL;
	rpi_localclient *localClient;

	client_p = BlockHeapAlloc(client_heap);

	client_p->v = &client_vtable;
	...
	if(from == NULL)
	{
		...
		localClient = (struct rpi_localclient *) BlockHeapAlloc(lclient_heap);
...
static void
free_local_client(struct rpi_client *client_p)
{
...
void client_dtor(void *o)
{
	rpi_client *client_p = o;
	s_assert(NULL != client_p);
	s_assert(me != client_p);
...
check_pings_list(dlink_list * list)
{
	...
	struct rpi_client *client_p;	/* current local client_p being examined */
		...
		if(IsPerson(client_p))
		{
			   ...
			   ((CurrentTime - client_p->user->last) > GlobalSetOptions.idletime))
			{
				...
				sendto_realops_flags(UMODE_ALL, L_ALL,
						     ...
						     client_p->v->get_name(client_p, HIDE_IP));
			    ...
			    && (client_p->flags & FLAGS_PINGSENT)))
			{
				if(IsAnyServer(client_p))
				{
					sendto_realops_flags(UMODE_ALL, L_ALL,
							     ...
							     client_p->v->server_name(client_p, HIDE_IP));
					ilog(L_SERVER,
					     "No response from %s, closing link",
					     client_p->v->log_name(client_p, HIDE_IP));
				}
...
static void
notify_banned_client(struct rpi_client *client_p, struct ConfItem *aconf, int ban)
{
 ...
 */
static void
update_client_exit_stats(struct rpi_client *client_p)
{
 ...
 */
static void
release_client_state(struct rpi_client *client_p)
{
 ...
 */
static void
remove_client_from_list(struct rpi_client *client_p)
{
	s_assert(NULL != client_p);
 ...
 */
struct rpi_client *
find_chasing(struct rpi_client *source_p, const char *user, int *chasing)
{
	struct rpi_client *who;
...
const char *
rpi_client_get_name(struct rpi_client *client, int showip)
{
...
const char *
rpi_client_server_name(struct rpi_client *target_p, int showip)
{
	static char nbuf[HOSTLEN * 2 + USERLEN + 5];
 ...
 */
const char *
rpi_client_log_name(struct rpi_client *target_p, int showip)
{
	static char nbuf[HOSTLEN * 2 + USERLEN + 5];
...
*/
static void
recurse_send_quits(struct rpi_client *client_p, struct rpi_client *source_p,
		   struct rpi_client *to, const char *comment,
		   const char *myname)
{
	struct rpi_client *target_p;
 ...
 */
static void
recurse_remove_clients(struct rpi_client *source_p, const char *comment)
{
	struct rpi_client *target_p;
	dlink_node *ptr, *ptr_next;
...
*/
static void
remove_dependents(struct rpi_client *client_p,
		  struct rpi_client *source_p,
		  struct rpi_client *from, const char *comment, const char *comment1)
{
	struct rpi_client *to;
...
static inline void
call_local_exit_hook(struct rpi_client *client_p, const char *comment)
{
...
static inline void
call_unknown_exit_hook(struct rpi_client *client_p, const char *comment)
{
	struct exit_client_hook e_hook;
	e_hook.client_p = client_p;
	strlcpy(e_hook.exit_message, comment, sizeof(e_hook.exit_message));
 	...
 	DLINK_FOREACH_SAFE(ptr, next, abort_list.head)
 	{
 		...
 		s_assert(*((unsigned long*)abt->client) != 0xdeadbeef); /* This is lame but its a debug thing */
 	 	...
 	 	if(!IsPerson(abt->client) && !IsUnknown(abt->client))
 	 	 	sendto_realops_flags(UMODE_ALL, L_ALL,
  	 	 	                     "Closing link to %s: %s",
   	 	 	                     abt->client->v->server_name(abt->client, HIDE_IP), abt->notice);
 ...
 */
void
rpi_client_dead_link(struct rpi_client *client_p)
{
	struct abort_client *abt;
...
static inline void
exit_generic_client(struct rpi_client *client_p, struct rpi_client *source_p, struct rpi_client *from,
		   const char *comment)
{
	dlink_node *ptr, *next_ptr;
	if(IsDead(source_p))
	...
	source_p->v->del_all_accepts(source_p);
...
static int
exit_remote_client(struct rpi_client *client_p, struct rpi_client *source_p, struct rpi_client *from,
		   const char *comment)
{
	if(IsDead(source_p))
		return -1;
...
static int
exit_unknown_client(struct rpi_client *client_p, struct rpi_client *source_p, struct rpi_client *from,
		  const char *comment)
{
	if(IsDead(source_p))
		return -1;
	...
	source_p->v->close_connection(source_p);
...
static int
exit_remote_server(struct rpi_client *client_p, struct rpi_client *source_p, struct rpi_client *from,
		  const char *comment)
{
...
static int
qs_server(struct rpi_client *client_p, struct rpi_client *source_p, struct rpi_client *from,
		  const char *comment)
{
	struct rpi_client *target_p;
...
static int
exit_local_server(struct rpi_client *client_p, struct rpi_client *source_p, struct rpi_client *from,
		  const char *comment)
{
	static char comment1[(HOSTLEN*2)+2];
...
static int
exit_local_client(struct rpi_client *client_p, struct rpi_client *source_p, struct rpi_client *from,
		  const char *comment)
{
...
*/
int
exit_client(struct rpi_client *client_p,	/* The local client originating the
					 * exit or NULL, if this exit is
					 * generated by this server for
					 * internal reasons.
					 * This will not get any of the
					 * generated messages. */
	    struct rpi_client *source_p,	/* Client exiting */
	    struct rpi_client *from,	/* Client firing off this Exit,
					 * never NULL! */
...
count_local_client_memory(size_t * count, size_t * local_client_memory_used)
{
	...
	*local_client_memory_used = lusage + (*count * (sizeof(MemBlock) + sizeof(struct rpi_client)));
}
...
count_remote_client_memory(size_t * count, size_t * remote_client_memory_used)
{
	...
	*count = rcount - lcount;
	*remote_client_memory_used = *count * (sizeof(MemBlock) + sizeof(struct rpi_client));
}
 ...
 */
int
rpi_client_accept_message(struct rpi_client *source, struct rpi_client *target)
{
 ...
 */
void
rpi_client_del_from_accept(struct rpi_client *source, struct rpi_client *target)
{
	dlink_node *ptr;
	dlink_node *ptr2;
	dlink_node *next_ptr;
 ...
 */
void
rpi_client_del_all_accepts(struct rpi_client *client_p)
{
	dlink_node *ptr;
	dlink_node *next_ptr;
	struct rpi_client *target_p;
	...
	DLINK_FOREACH_SAFE(ptr, next_ptr, client_p->allow_list.head)
	{
		target_p = ptr->data;
		if(target_p != NULL)
			target_p->v->del_from_accept(target_p, client_p);
	}
	...
	DLINK_FOREACH_SAFE(ptr, next_ptr, client_p->on_allow_list.head)
	{
		target_p = ptr->data;
		if(target_p != NULL)
			client_p->v->del_from_accept(client_p, target_p);
	}
}
...
int
rpi_client_show_ip(struct rpi_client *source_p, struct rpi_client *target_p)
{
	int s, t, res;
...
initUser(void)
{
	user_heap = BlockHeapCreate(sizeof(struct rpi_localclient), USER_HEAP_SIZE);
	if(!user_heap)
		outofmemory();
}
 ...
 */
struct rpi_user *
make_user(struct rpi_client *client_p)
{
	...
	if(!user)
	{
		user = (struct rpi_user *) BlockHeapAlloc(user_heap);
		user->refcnt = 1;
		client_p->user = user;
	}
	return user;
 ...
 */
struct rpi_server *
make_server(struct rpi_client *client_p)
{
	struct rpi_server *serv = client_p->serv;

	if(!serv)
	{
		serv = (struct rpi_server *) MyMalloc(sizeof(struct rpi_server));
		client_p->serv = serv;
	}
 ...
 */
void
free_user(struct rpi_user *user, struct rpi_client *client_p)
{
	if(--user->refcnt <= 0)
	{
		if(user->away)
...
void
error_exit_client(struct rpi_client *client_p, int error)
{
	/*
	...
	if(IsServer(client_p) || IsHandshake(client_p))
	{
		...
		if(error == 0)
		{
			sendto_realops_flags(UMODE_ALL, L_ALL,
					     ...
					     client_p->v->server_name(client_p, SHOW_IP));

			ilog(L_SERVER, "Server %s closed the connection",
			     client_p->v->log_name(client_p, SHOW_IP));
		}
		else
		{
