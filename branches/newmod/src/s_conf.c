...
static int verify_access(struct rpi_client *client_p, const char *username);
static int attach_iline(struct rpi_client *, struct ConfItem *);
 ...
 */
int
check_client(struct rpi_client *client_p, struct rpi_client *source_p, const char *username)
{
	int i;
	...
	case TOO_MANY_LOCAL:
		sendto_realops_flags(UMODE_FULL, L_ALL,
				     ...
				     source_p->v->get_name(source_p, SHOW_IP));

		ilog(L_FUSER, "Too many local connections from %s",
		     source_p->v->log_name(source_p, SHOW_IP));
	...
	case I_LINE_FULL:
		...
		ilog(L_FUSER, "Too many connections from %s.", source_p->v->log_name(source_p, SHOW_IP));
 ...
 */
static int
verify_access(struct rpi_client *client_p, const char *username)
{
	struct ConfItem *aconf;
...
static int
add_ip_limit(struct rpi_client *client_p, struct ConfItem *aconf)
{
...
static void
remove_ip_limit(struct rpi_client *client_p, struct ConfItem *aconf)
{
	patricia_node_t *pnode;
 ...
 */
static int
attach_iline(struct rpi_client *client_p, struct ConfItem *aconf)
{
 ...
 */
int
detach_conf(struct rpi_client *client_p)
{
	struct ConfItem *aconf;
 ...
 */
int
attach_conf(struct rpi_client *client_p, struct ConfItem *aconf)
{
	if(IsIllegal(aconf))
		return (NOT_AUTHORISED);
...
/* const char* get_oper_name(struct rpi_client *client_p)
 ...
 */
char *
get_oper_name(struct rpi_client *client_p)
{
	/* +5 for !,@,{,} and null */
...
void
get_printable_kline(struct rpi_client *source_p, struct ConfItem *aconf,
 ...
 */
void
write_confitem(KlineType type, struct rpi_client *source_p, char *user,
