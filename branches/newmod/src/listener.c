 ...
 */
void
show_ports(struct rpi_client *source_p)
{
...
add_connection(struct Listener *listener, int fd, struct sockaddr_storage *sai)
{
	...
	if(!comm_set_nb(new_client->localClient->fd))
		report_error(NONB_ERROR_MSG, new_client->v->get_name(new_client, SHOW_IP),
			     new_client->v->log_name(new_client, SHOW_IP), errno);
