...
static int mo_operwall(struct rpi_client *, struct rpi_client *, int, const char **);
static int ms_operwall(struct rpi_client *, struct rpi_client *, int, const char **);
static int ms_wallops(struct rpi_client *, struct rpi_client *, int, const char **);
 ...
 */
static int
mo_operwall(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[])
{
 ...
 */
static int
ms_operwall(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[])
{
	sendto_server(client_p, NULL, CAP_TS6, NOCAPS, ":%s OPERWALL :%s",
		      use_id(source_p), parv[1]);
	sendto_server(client_p, NULL, NOCAPS, CAP_TS6, ":%s OPERWALL :%s",
 ...
 */
static int
ms_wallops(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[])
{
