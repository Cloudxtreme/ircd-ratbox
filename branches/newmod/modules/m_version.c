...
static char *confopts(struct rpi_client *source_p);

static int m_version(struct rpi_client *, struct rpi_client *, int, const char **);
static int mo_version(struct rpi_client *, struct rpi_client *, int, const char **);
 ...
 */
static int
m_version(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[])
{
 ...
 */
static int
mo_version(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[])
{
	if(hunt_server(client_p, source_p, ":%s VERSION :%s", 1, parc, parv) == HUNTED_ISME)
	{
		sendto_one_numeric(source_p, RPL_VERSION, form_str(RPL_VERSION),
 ...
 */
static char *
confopts(struct rpi_client *source_p)
{
