...
static int m_map(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[]);
static int mo_map(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[]);
...
static void dump_map(struct rpi_client *client_p, struct rpi_client *root, char *pbuf);
...
*/
static int
m_map(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[])
{
...
*/
static int
mo_map(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[])
{
	dump_map(client_p, me, buf);
	sendto_one(client_p, form_str(RPL_MAPEND), me->name, client_p->name);
...
*/
static void
dump_map(struct rpi_client *client_p, struct rpi_client *root_p, char *pbuf)
{
	int cnt = 0, i = 0, len;
	struct rpi_client *server_p;
	dlink_node *ptr;
	*pbuf = '\0';
