...
#include "stdinc.h"
#include "tools.h"
#include "rpi/client.h"		/* client struct */
...
static int mr_server(struct rpi_client *, struct rpi_client *, int, const char **);
static int ms_server(struct rpi_client *, struct rpi_client *, int, const char **);
static int ms_sid(struct rpi_client *, struct rpi_client *, int, const char **);
...
struct rpi_client *server_exists(const char *);
static int set_server_gecos(struct rpi_client *, const char *);
 ...
 */
static int
mr_server(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[])
{
	char info[REALLEN + 1];
	const char *name;
	...
	case -2:
		sendto_realops_flags(UMODE_ALL, L_ALL,
				...
				client_p->v->server_name(client_p, HIDE_IP), name);
	...
	case -4:
		sendto_realops_flags(UMODE_ALL, L_ALL,
				     ...
				     name, client_p->v->server_name(client_p, HIDE_IP));
 ...
 */
static int
ms_server(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[])
{
	char info[REALLEN + 1];
	...
	if((target_p = server_exists(name)))
	{
		...
		sendto_realops_flags(UMODE_ALL, L_ALL,
				     ...
				     client_p->v->server_name(client_p, SHOW_IP), name);
	...
	target_p = new("client", 0, client_p);
...
static int
ms_sid(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[])
{
	struct rpi_client *target_p;
	struct remote_conf *hub_p;
	dlink_node *ptr;
	...
	if((target_p = server_exists(parv[1])) != NULL)
	{
		...
		sendto_realops_flags(UMODE_ALL, L_ALL,
				     "Link %s cancelled, server %s already exists",
				     client_p->v->server_name(client_p, SHOW_IP), parv[1]);
	...
	if((target_p = find_id(parv[3])) != NULL)
	{
		...
		sendto_realops_flags(UMODE_ALL, L_ALL,
				     ...
				     client_p->v->server_name(client_p, SHOW_IP), parv[3]);
		exit_client(NULL, client_p, me, "Server Exists");
		return 0;
	}
 ...
 */
static int
set_server_gecos(struct rpi_client *client_p, const char *info)
{
