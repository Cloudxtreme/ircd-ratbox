...
static int ms_squit(struct rpi_client *, struct rpi_client *, int, const char **);
static int mo_squit(struct rpi_client *, struct rpi_client *, int, const char **);
...
static struct squit_parms *find_squit(struct rpi_client *client_p,
				      struct rpi_client *source_p, const char *server);
 ...
 */
static int
mo_squit(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[])
{
	...
	if((found_squit = find_squit(client_p, source_p, parv[1])))
	{
		if(MyConnect(found_squit->target_p))
		{
			sendto_realops_flags(UMODE_ALL, L_ALL,
					     ...
					     source_p->v->get_name(source_p, HIDE_IP), comment);
			ilog(L_SERVER, "Received SQUIT %s from %s (%s)",
			     found_squit->target_p->name, source_p->v->log_name(source_p, HIDE_IP),
			     comment);
		}
 ...
 */
static int
ms_squit(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[])
{
	struct rpi_client *target_p;
	const char *comment = (parc > 2 && parv[2]) ? parv[2] : client_p->name;
...
find_squit(struct rpi_client *client_p, struct rpi_client *source_p, const char *server)
{
	static struct squit_parms found_squit;
	struct rpi_client *target_p = NULL;
	struct rpi_client *p;
	dlink_node *ptr;
