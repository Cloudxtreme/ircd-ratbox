...
static int m_message(int, const char *, struct rpi_client *, struct rpi_client *, int, const char **);
static int m_privmsg(struct rpi_client *, struct rpi_client *, int, const char **);
static int m_notice(struct rpi_client *, struct rpi_client *, int, const char **);
...
static int build_target_list(int p_or_n, const char *command,
			     ...
			     struct rpi_client *source_p, const char *nicks_channels, const char *text);

static int flood_attack_client(int p_or_n, struct rpi_client *source_p, struct rpi_client *target_p);
static int flood_attack_channel(int p_or_n, struct rpi_client *source_p,
				struct Channel *chptr, char *chname);
static struct rpi_client *find_userhost(const char *, const char *, int *);
...
static void msg_channel(int p_or_n, const char *command,
			struct rpi_client *client_p,
			struct rpi_client *source_p, struct Channel *chptr, const char *text);
...
static void msg_client(int p_or_n, const char *command,
		       struct rpi_client *source_p, struct rpi_client *target_p, const char *text);

static void handle_special(int p_or_n, const char *command,
			 struct rpi_client *client_p, struct rpi_client *source_p, const char *nick, const char *text);
...
static int
m_privmsg(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[])
{
	return m_message(PRIVMSG, "PRIVMSG", client_p, source_p, parc, parv);
}

static int
m_notice(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[])
{
...
m_message(int p_or_n,
	  const char *command,
	  struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[])
{
	int i;
	...
	for (i = 0; i < ntargets; i++)
	{
		...
		case ENTITY_CLIENT:
			msg_client(p_or_n, command, source_p,
				   (struct rpi_client *) targets[i].ptr, parv[2]);
			break;
		}
	}
...
static int
build_target_list(int p_or_n, const char *command, struct rpi_client *client_p,
		  struct rpi_client *source_p, const char *nicks_channels, const char *text)
{
	int type;
...
msg_channel(int p_or_n, const char *command,
	    struct rpi_client *client_p, struct rpi_client *source_p, struct Channel *chptr, const char *text)
{
	int result;
 ...
 */
static void
msg_channel_flags(int p_or_n, const char *command, struct rpi_client *client_p,
		  struct rpi_client *source_p, struct Channel *chptr, int flags, const char *text)
{
	int type;
	char c;
 ...
 *		  say NOTICE must not auto reply
 *		- pointer to command, "PRIVMSG" or "NOTICE"
 * 		- pointer to source_p source (struct rpi_client *)
 *		- pointer to target_p target (struct rpi_client *)
 *		- pointer to text
 * output	- NONE
 * side effects	- message given channel either chanop or voice
 */
	   ...
	   struct rpi_client *source_p, struct rpi_client *target_p, const char *text)
{
	...
	if(MyClient(target_p))
	{
		...
		if(!IsServer(source_p) && IsSetCallerId(target_p))
		{
			...
			if(source_p->v->accept_message(source_p, target_p) || IsOper(source_p))
			{
 ...
 */
static int
flood_attack_client(int p_or_n, struct rpi_client *source_p, struct rpi_client *target_p)
{
	int delta;
 ...
 */
static int
flood_attack_channel(int p_or_n, struct rpi_client *source_p, struct Channel *chptr, char *chname)
{
	int delta;
 ...
 */
static void
handle_special(int p_or_n, const char *command, struct rpi_client *client_p,
	     struct rpi_client *source_p, const char *nick, const char *text)
{
	struct rpi_client *target_p;
	char *host;
	char *server;
