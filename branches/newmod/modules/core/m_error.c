...
static int m_error(struct rpi_client *, struct rpi_client *, int, const char **);
static int ms_error(struct rpi_client *, struct rpi_client *, int, const char **);
 ...
 */
int
m_error(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[])
{
	const char *para;
	...
	ilog(L_SERVER, "Received ERROR message from %s: %s",
		source_p->v->log_name(source_p, SHOW_IP), para);

	if(IsAnyServer(client_p) && ConfigFileEntry.hide_error_messages < 2)
	{
		sendto_realops_flags(UMODE_ALL, L_ADMIN,
				"ERROR :from %s -- %s",
				client_p->v->server_name(client_p, HIDE_IP), para);
...
static int
ms_error(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[])
{
	const char *para;
