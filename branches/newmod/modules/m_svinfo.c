...
static int ms_svinfo(struct rpi_client *, struct rpi_client *, int, const char **);
 ...
 */
static int
ms_svinfo(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[])
{
	signed int deltat;
	time_t theirtime;
	...
	if(TS_CURRENT < atoi(parv[2]) || atoi(parv[1]) < TS_MIN)
	{
		...
		sendto_realops_flags(UMODE_ALL, L_ALL,
				     ...
				     source_p->v->server_name(source_p, SHOW_IP), parv[1], parv[2]);
	...
	if(deltat > ConfigFileEntry.ts_max_delta)
	{
		sendto_realops_flags(UMODE_ALL, L_ALL,
				     ...
				     source_p->v->server_name(source_p, SHOW_IP),
				     (long) CurrentTime, (long) theirtime, deltat);
		ilog(L_SERVER,
		     "Link %s dropped, excessive TS delta"
		     " (my TS=%ld, their TS=%ld, delta=%d)",
		     source_p->v->log_name(source_p, SHOW_IP), (long) CurrentTime, (long) theirtime, deltat);
