...
static int m_accept(struct rpi_client *, struct rpi_client *, int, const char **);
static void build_nicklist(struct rpi_client *, char *, char *, const char *);

static void add_accept(struct rpi_client *, struct rpi_client *);
static void list_accepts(struct rpi_client *);
 ...
 */
static int
m_accept(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[])
{
	char *nick;
	char *p = NULL;
	...
	for (nick = strtoken(&p, delbuf, ","); nick != NULL; nick = strtoken(&p, NULL, ","))
	{
		...
		if(!target_p->v->accept_message(target_p, source_p))
		{
		...
		target_p->v->del_from_accept(target_p, source_p);
	}
 ...
 */
static void
build_nicklist(struct rpi_client *source_p, char *addbuf, char *delbuf, const char *nicks)
{
	char *name;
	char *p;
	int lenadd;
 ...
 */
static void
add_accept(struct rpi_client *source_p, struct rpi_client *target_p)
{
 ...
 */
static void
list_accepts(struct rpi_client *source_p)
{
	dlink_node *ptr;
	struct rpi_client *target_p;
	char nicks[BUFSIZE];
