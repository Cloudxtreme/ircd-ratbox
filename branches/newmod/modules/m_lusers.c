...
static int m_lusers(struct rpi_client *, struct rpi_client *, int, const char **);
static int ms_lusers(struct rpi_client *, struct rpi_client *, int, const char **);
 ...
 */
static int
m_lusers(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[])
{
 ...
 */
static int
ms_lusers(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[])
{
	if(parc > 2)
	{
		if(hunt_server(client_p, source_p, ":%s LUSERS %s :%s", 2, parc, parv)
