...
static void send_conf_options(struct rpi_client *source_p);
static void send_birthdate_online_time(struct rpi_client *source_p);
static void send_info_text(struct rpi_client *source_p);
static void info_spy(struct rpi_client *);

static int m_info(struct rpi_client *, struct rpi_client *, int, const char **);
static int mo_info(struct rpi_client *, struct rpi_client *, int, const char **);
...
*/
static int
m_info(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[])
{
...
*/
static int
mo_info(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[])
{
	if(hunt_server(client_p, source_p, ":%s INFO :%s", 1, parc, parv) == HUNTED_ISME)
	{
		info_spy(source_p);
 ...
 */
static void
send_info_text(struct rpi_client *source_p)
{
 ...
 */
static void
send_birthdate_online_time(struct rpi_client *source_p)
{
 ...
 */
static void
send_conf_options(struct rpi_client *source_p)
{
	Info *infoptr;
	int i = 0;
 ...
 */
static void
info_spy(struct rpi_client *source_p)
{
