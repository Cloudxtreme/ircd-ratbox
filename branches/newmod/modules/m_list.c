...
static int m_list(struct rpi_client *, struct rpi_client *, int, const char **);
static int mo_list(struct rpi_client *, struct rpi_client *, int, const char **);
...
static void list_all_channels(struct rpi_client *source_p);
static void list_named_channel(struct rpi_client *source_p, const char *name);
 ...
 */
static int
m_list(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[])
{
 ...
 */
static int
mo_list(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[])
{
	/* If no arg, do all channels *whee*, else just one channel */
	if(parc < 2 || EmptyString(parv[1]))
		list_all_channels(source_p);
 ...
 */
static void
list_all_channels(struct rpi_client *source_p)
{
 ...
 */
static void
list_named_channel(struct rpi_client *source_p, const char *name)
{
	struct Channel *chptr;
	char *p;
