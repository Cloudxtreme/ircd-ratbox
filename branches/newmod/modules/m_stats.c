...
static int m_stats (struct rpi_client *, struct rpi_client *, int, const char **);
...
static void stats_l_list(struct rpi_client *s, const char *, int, int, dlink_list *, char);
static void stats_l_client(struct rpi_client *source_p, struct rpi_client *target_p,
				char statchar);

static void stats_spy(struct rpi_client *, char, const char *);
static void stats_p_spy(struct rpi_client *);
...
static void stats_adns_servers (struct rpi_client *);
static void stats_delay(struct rpi_client *);
static void stats_hash(struct rpi_client *);
static void stats_connect (struct rpi_client *);
static void stats_tdeny (struct rpi_client *);
static void stats_deny (struct rpi_client *);
static void stats_exempt (struct rpi_client *);
static void stats_events (struct rpi_client *);
static void stats_glines (struct rpi_client *);
static void stats_pending_glines (struct rpi_client *);
static void stats_hubleaf (struct rpi_client *);
static void stats_auth (struct rpi_client *);
static void stats_tklines (struct rpi_client *);
static void stats_klines (struct rpi_client *);
static void stats_messages (struct rpi_client *);
static void stats_oper (struct rpi_client *);
static void stats_operedup (struct rpi_client *);
static void stats_ports (struct rpi_client *);
static void stats_resv (struct rpi_client *);
static void stats_usage (struct rpi_client *);
static void stats_tstats (struct rpi_client *);
static void stats_uptime (struct rpi_client *);
static void stats_shared (struct rpi_client *);
static void stats_servers (struct rpi_client *);
static void stats_gecos (struct rpi_client *);
static void stats_class (struct rpi_client *);
static void stats_memory (struct rpi_client *);
static void stats_servlinks (struct rpi_client *);
static void stats_ltrace (struct rpi_client *, int, const char **);
static void stats_ziplinks (struct rpi_client *);

static void report_tklines (struct rpi_client *, dlink_list *);
 ...
 */
static int
m_stats(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[])
{
...
static void
stats_adns_servers (struct rpi_client *source_p)
{
	report_adns_servers (source_p);
}

static void
stats_delay(struct rpi_client *source_p)
{
	struct nd_entry *nd;
	dlink_node *ptr;
	int i;
...
static void
stats_hash(struct rpi_client *source_p)
{
	hash_stats(source_p);
}

static void
stats_connect(struct rpi_client *source_p)
{
	static char buf[5];
 ...
 */
static void
stats_tdeny (struct rpi_client *source_p)
{
 ...
 */
static void
stats_deny (struct rpi_client *source_p)
{
	char *host, *pass, *user, *oper_reason;
	struct AddressRec *arec;
	struct ConfItem *aconf;
 ...
 */
static void
stats_exempt(struct rpi_client *source_p)
{
...
static void
stats_events (struct rpi_client *source_p)
{
 ...
 */
static void
stats_pending_glines (struct rpi_client *source_p)
{
 ...
 */
static void
stats_glines (struct rpi_client *source_p)
{
	if(ConfigFileEntry.glines)
	{
...
static void
stats_hubleaf (struct rpi_client *source_p)
{
...
static void
stats_auth (struct rpi_client *source_p)
{
...
static void
stats_tklines (struct rpi_client *source_p)
{
	/* Oper only, if unopered, return ERR_NOPRIVS */
...
static void
report_tklines(struct rpi_client *source_p, dlink_list * tkline_list)
{
	struct ConfItem *aconf;
	dlink_node *ptr;
	char *host;
...
static void
stats_klines (struct rpi_client *source_p)
{
	/* Oper only, if unopered, return ERR_NOPRIVS */
	if((ConfigFileEntry.stats_k_oper_only == 2) && !IsOper (source_p))
		sendto_one_numeric(source_p, ERR_NOPRIVILEGES,
...
static void
stats_messages(struct rpi_client *source_p)
{
	report_messages(source_p);
}

static void
stats_oper(struct rpi_client *source_p)
{
 ...
 */
static void
stats_operedup (struct rpi_client *source_p)
{
...
static void
stats_ports (struct rpi_client *source_p)
{
...
static void
stats_resv(struct rpi_client *source_p)
{
	struct ConfItem *aconf;
	dlink_node *ptr;
...
static void
stats_usage (struct rpi_client *source_p)
{
#ifndef __VMS
	struct rusage rus;
#endif
...
static void
stats_tstats (struct rpi_client *source_p)
{
	tstats (source_p);
}

static void
stats_uptime (struct rpi_client *source_p)
{
	time_t now;
...
static void
stats_shared (struct rpi_client *source_p)
{
 ...
 */
static void
stats_servers (struct rpi_client *source_p)
{
	struct rpi_client *target_p;
	dlink_node *ptr;
	time_t seconds;
...
static void
stats_gecos(struct rpi_client *source_p)
{
	struct ConfItem *aconf;
	dlink_node *ptr;
...
static void
stats_class(struct rpi_client *source_p)
{
...
static void
stats_memory (struct rpi_client *source_p)
{
	count_memory (source_p);
}

static void
stats_ziplinks (struct rpi_client *source_p)
{
	dlink_node *ptr;
	struct rpi_client *target_p;
	int sent_data = 0;
...
static void
stats_servlinks (struct rpi_client *source_p)
{
	...
	DLINK_FOREACH (ptr, serv_list.head)
	{
		...
		sendto_one(source_p, Sformat,
			...
			target_p->v->server_name(target_p, SHOW_IP),
			(int) linebuf_len (&target_p->localClient->buf_sendq),
...
static void
stats_ltrace(struct rpi_client *source_p, int parc, const char *parv[])
{
	int doall = 0;
	int wilds = 0;
	const char *name;
	...
	if(!doall && !wilds)
	{
		struct rpi_client *target_p = find_named_person(name);
...
static void
stats_l_list(struct rpi_client *source_p, const char *name, int doall, int wilds,
...
void
stats_l_client(struct rpi_client *source_p, struct rpi_client *target_p,
		char statchar)
{
	...
	else if(IsIPSpoof(target_p))
	{
		sendto_one_numeric(source_p, RPL_STATSDEBUG, Lformat,
				   ...
				   IsOper(source_p) ?
				    (IsUpper(statchar) ?
				     target_p->v->get_name(target_p, SHOW_IP) :
				     target_p->v->get_name(target_p, HIDE_IP)) :
#endif
				    target_p->v->get_name(target_p, MASK_IP),
				    (int) linebuf_len(&target_p->localClient->buf_sendq),
				    (int) target_p->localClient->sendM,
				    (int) target_p->localClient->sendK,
				    (int) target_p->localClient->receiveM,
 ...
 */
static void
stats_spy(struct rpi_client *source_p, char statchar, const char *name)
{
 ...
 */
static void
stats_p_spy (struct rpi_client *source_p)
{
	struct hook_spy_data data;
