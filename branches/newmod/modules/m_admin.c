...
static int m_admin(struct rpi_client *, struct rpi_client *, int, const char **);
static int mr_admin(struct rpi_client *, struct rpi_client *, int, const char **);
static int ms_admin(struct rpi_client *, struct rpi_client *, int, const char **);
static void do_admin(struct rpi_client *source_p);

static void admin_spy(struct rpi_client *);
 ...
 */
static int
mr_admin(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[])
{
 ...
 */
static int
m_admin(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[])
{
	static time_t last_used = 0L;
 ...
 */
static int
ms_admin(struct rpi_client *client_p, struct rpi_client *source_p, int parc, const char *parv[])
{
	if(hunt_server(client_p, source_p, ":%s ADMIN :%s", 1, parc, parv) != HUNTED_ISME)
		return 0;
 ...
 */
static void
do_admin(struct rpi_client *source_p)
{
	const char *myname;
	const char *nick;
 ...
 */
static void
admin_spy(struct rpi_client *source_p)
{
