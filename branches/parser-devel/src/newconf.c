...
int 	conf_call_set(char *item, conf_parm_t *value, int type)
{
	struct ConfEntry *cf;
	conf_parm_t *cp;

	if ((cf = find_conf_item(item)) == NULL)
	{
		conf_report_error("Non-existant configuration setting %s.", (char*)item);
		return -1;
	}
	...
	{
		conf_report_error("Option %s does not take a list of values.", item);
		return -1;
	}
	...
	{
		conf_report_error("Wrong type for %s (expected %s, got %s)",
				(char*)item, conf_strtype(cf->cf_type),
...
int 	add_conf_item(char *name, int type, void (*func)(void*))
{
	struct ConfEntry *cf;

        if((cf = find_conf_item(name)) != NULL)
                return -1;
	...
	dlinkAddAlloc(cf, &conf_items);

	return 0;
}

int     remove_conf_item(char *name)
{
        struct ConfEntry *cf;
        dlink_node *ptr;
	...
	if ((cf = find_conf_item(name)) == NULL)
        ...
        if ((ptr = dlinkFind(&conf_items, cf)) == NULL)
                return -1;
...
struct 	ConfEntry *find_conf_item(const char* name)
{
	dlink_node *d;
 	struct ConfEntry* cf;
...
void	newconf_init()
{
	add_conf_item("modules::path", CF_STRING, conf_set_modules_path);
	add_conf_item("modules::module", CF_STRING, conf_set_modules_module);

	add_conf_item("serverinfo::rsa_private_key_file", CF_STRING,
			conf_set_serverinfo_rsa_private_key_file);
	add_conf_item("serverinfo::name", CF_STRING,
			conf_set_serverinfo_name);
	add_conf_item("serverinfo::description", CF_STRING,
			conf_set_serverinfo_description);
	add_conf_item("serverinfo::network_name", CF_STRING,
			conf_set_serverinfo_network_name);
	add_conf_item("serverinfo::network_desc", CF_STRING,
			conf_set_serverinfo_network_desc);
	add_conf_item("serverinfo::vhost", CF_STRING,
			conf_set_serverinfo_vhost);
	add_conf_item("serverinfo::vhost6", CF_STRING,
	...
	add_conf_item("admin::name", CF_STRING, conf_set_admin_name);
	add_conf_item("admin::description", CF_STRING, 
			conf_set_admin_description);
	add_conf_item("admin::email", CF_STRING, conf_set_admin_email);

	add_conf_item("logging::path", CF_STRING, conf_set_logging_path);
	add_conf_item("logging::oper_log", CF_STRING, 
			conf_set_logging_oper_log);
	add_conf_item("logging::gline_log", CF_STRING,
			conf_set_logging_gline_log);
	add_conf_item("logging::log_level", CF_STRING,
			conf_set_logging_log_level);

	add_conf_item("operator::name", CF_STRING, conf_set_oper_name);
	add_conf_item("operator::user", CF_STRING, conf_set_oper_user);
	add_conf_item("operator::password", CF_STRING, conf_set_oper_password);
	add_conf_item("operator::class", CF_STRING, conf_set_oper_class);
	...
	add_conf_item("operator::rsa_public_key_file", CF_STRING,
			conf_set_oper_rsa_public_key_file);
	add_conf_item("operator::flags", CF_STRING | CF_FLIST,
			conf_set_oper_flags);

	add_conf_item("class::name", CF_STRING, conf_set_class_name);
	...
	add_conf_item("listen::ip", CF_STRING, conf_set_listen_address);
	add_conf_item("listen::host", CF_STRING, conf_set_listen_address);

	add_conf_item("auth::user", CF_STRING, conf_set_auth_user);
	add_conf_item("auth::password", CF_STRING, conf_set_auth_passwd);
	add_conf_item("auth::class", CF_STRING, conf_set_auth_class);
	...
	add_conf_item("auth::spoof", CF_STRING, conf_set_auth_spoof);
	...
	add_conf_item("auth::redirserv", CF_STRING, conf_set_auth_redir_serv);
	...
	add_conf_item("resv::reason", CF_STRING, conf_set_resv_reason);
	add_conf_item("resv::channel", CF_STRING, conf_set_resv_channel);
	add_conf_item("resv::nick", CF_STRING, conf_set_resv_nick);

	add_conf_item("shared::name", CF_STRING, conf_set_shared_name);
	add_conf_item("shared::user", CF_STRING, conf_set_shared_user);
	add_conf_item("shared::kline", CF_YESNO, conf_set_shared_kline);
	add_conf_item("shared::unkline", CF_YESNO, conf_set_shared_unkline);

	add_conf_item("connect::name", CF_STRING, conf_set_connect_name);
	add_conf_item("connect::host", CF_STRING, conf_set_connect_host);
	add_conf_item("connect::send_password", CF_STRING, 
			conf_set_connect_send_password);
	add_conf_item("connect::accept_password", CF_STRING,
	...
	add_conf_item("connect::fakename", CF_STRING, conf_set_connect_fakename);
	add_conf_item("connect::hub_mask", CF_STRING, conf_set_connect_hub_mask);
	add_conf_item("connect::leaf_mask", CF_STRING, conf_set_connect_leaf_mask);
	add_conf_item("connect::class", CF_STRING, conf_set_connect_class);
	...
	add_conf_item("connect::rsa_public_key_file", CF_STRING,
			conf_set_connect_rsa_public_key_file);
	add_conf_item("connect::cipher_preference", CF_STRING,
			conf_set_connect_cipher_preference);

	add_conf_item("kill::user", CF_STRING, conf_set_kill_user);
	add_conf_item("kill::reason", CF_STRING, conf_set_kill_reason);

	add_conf_item("deny::ip", CF_STRING, conf_set_deny_ip);
	add_conf_item("deny::reason", CF_STRING, conf_set_deny_reason);

	add_conf_item("exempt::ip", CF_STRING, conf_set_exempt_ip);

	add_conf_item("gecos::name", CF_STRING, conf_set_gecos_name);
	add_conf_item("gecos::reason", CF_STRING, conf_set_gecos_reason);
	...
	add_conf_item("general::message_locale", CF_STRING,
			conf_set_general_message_locale);
	add_conf_item("general::client_exit", CF_YESNO,
			conf_set_general_client_exit);
	add_conf_item("general::fname_userlog", CF_STRING,
			conf_set_general_fname_userlog);
	add_conf_item("general::fname_operlog", CF_STRING,
			conf_set_general_fname_operlog);
	add_conf_item("general::fname_foperlog", CF_STRING,
	...
	add_conf_item("general::egdpool_path", CF_STRING,
	...
	add_conf_item("general::servlink_path", CF_STRING,
	...
	add_conf_item("general::default_cipher_preference", CF_STRING,
