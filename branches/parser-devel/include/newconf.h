...
struct	HeirItem
{
	...
	char *		label;
	int		terminal;
	conf_parm_t *	item; /* only if terminal */
};
...
extern 		dlink_list 	conf_items;
	...
	int 	add_conf_item		(char *, int, void (*)(void *));
        int     remove_conf_item	(char *);
struct 	ConfEntry *find_conf_item	(const char*);
