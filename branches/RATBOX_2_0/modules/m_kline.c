...
ms_kline(struct Client *client_p, struct Client *source_p, int parc, const char *parv[])
{
	...
	/* ratbox1.5-3 and earlier contains a bug which allows opers to set
	 * remote klines without a reason.  Thus we can only require 5
	 * params in the msgtab (which splits if its wrong), when we
	 * actually need 6.  Test that here and just drop it. --anfl
	 */
		...
		const char *user, const char *host, const char *kreason)
{
	...
	{
		ircsnprintf(buffer, sizeof(buffer),
			"Temporary K-line %u min. - %s (%s)",
			(unsigned int) (tkline_time / 60), reason, current_date);
		DupString(aconf->passwd, buffer);
	...
	{
		ircsnprintf(buffer, sizeof(buffer), "%s (%s)",
				reason, current_date);
		DupString(aconf->passwd, buffer);
		apply_kline(source_p, aconf, reason, oper_reason, current_date);
	}
